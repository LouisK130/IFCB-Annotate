function insertData4(conn, old_classification, file)
    ...
    if ~isnan(tag_label)
        ...
        if tag_id < 0
            ...
            if isempty(error)
                ...
                fprintf('Failed to create tag label ''%s'' for ''%s''\n', tag_label, file);
                return;
            end
        end
    ...
    query = ['CREATE TEMP TABLE tmp_table ON COMMIT DROP AS SELECT bin, roi, classification_id ' ...
        'FROM classify_classification WITH NO DATA; ' ...
        'ALTER TABLE tmp_table ALTER COLUMN classification_id SET DEFAULT ''%d''; ' ...
        'COPY tmp_table (bin,roi) FROM ''%s'' CSV; ' ...
        'INSERT INTO classify_classification (bin, roi, classification_id) SELECT DISTINCT ON ' ...
    ...
    if ~isnan(tag_label)
        ...
        query = ['CREATE TEMP TABLE tmp_table ON COMMIT DROP AS SELECT bin, roi, tag_id ' ...
            'FROM classify_tag WITH NO DATA; ' ...
            'ALTER TABLE tmp_table ALTER COLUMN tag_id SET DEFAULT ''%d''; ' ...
            'COPY tmp_table (bin,roi) FROM ''%s'' CSV; ' ...
            'INSERT INTO classify_tag (bin, roi, tag_id) SELECT DISTINCT ON ' ...
